...
void main() {
  ...
  test('generic non-FHIR XML is not claimed as FHIR', () {
    final registry = ParserRegistry.standard();
    expect(registry.detect(u('<note><to>x</to></note>')),
  ...
  test('EDIFACT: release character escapes a separator', () {
    final r = registry.parse(u(
      "UNB+UNOA:1+ACME?+CO+RCP+200101:1200+1'UNH+1+INVOIC:D:96A:UN'UNT+2+1'",
    ));
  ...
  test('USMTF: sets parsed, MSGID surfaced; loose slashes not misclassified', () {
    final r = registry.parse(u(
      'EXER/TEST EXERCISE//\n'
    ...
    expect((r.metadata['setIds'] as List), containsAll(['EXER', 'MSGID', 'GENTEXT']));

    expect(registry.detect(u('see https://a/b and c/d/e for details')),
