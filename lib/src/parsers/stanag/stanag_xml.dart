...
class _XmlPeek {
  ...
  static XmlDocument parse(Uint8List bytes, String standard) {
    try {
      ...
      throw TextDecodingException('$standard XML is not valid UTF-8: ${e.message}');
    }
  }
}
...
final class Stanag4676TrackingParser implements StanagSubParser {
  ...
  static const _roots = {
    'TrackMessage', 'TrackMessageType', 'NITS', 'TrackCollection',
  };
  ...
  StanagParse parse(Uint8List bytes) {
    ...
    final points = doc.findAllElements('TrackPoint', namespace: '*').length +
...
final class Stanag4778BindingParser implements StanagSubParser {
  ...
  static const _roots = {
    'BindingInformation', 'BindingInformationType',
    'MetadataBindingContainer', 'OriginatorConfidentialityLabelledData',
  };
  ...
  StanagParse parse(Uint8List bytes) {
    final doc = _XmlPeek.parse(bytes, standard);
    final bindings = doc.findAllElements('Binding', namespace: '*').length +
        doc.findAllElements('MetadataBinding', namespace: '*').length;
    final hasLabel = doc
        .findAllElements('OriginatorConfidentialityLabel', namespace: '*')
        .isNotEmpty;
    final hasSignature =
        doc.findAllElements('Signature', namespace: '*').isNotEmpty;
